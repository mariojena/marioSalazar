...
def page_user(request, id):
    ...
    if user.is_authenticated:
        for post in Post.objects.filter(who = user_page).order_by("-date").values():
            # Get the profile of the person who posted
            person = post["who_id"]
            who_user = User.objects.values('id','username').filter(id=user_page.id).values()
            profile = Profile.objects.filter(person=user_page)

            # Check wether the post is public or if the user follows the person who posted it
            if post["private"] == False or user in profile.get().followers.all() or user.id == user_page.id:
